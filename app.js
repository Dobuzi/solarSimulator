...
if (!window.SimCore) {
  console.warn("SimCore not found. Falling back to bundled orbital math.");
  window.SimCore = {
    ORBIT_SCALE,
    DEG_TO_RAD,
    INCLINATION_SCALE,
    ...
    computeElements(planet, centuries) {
    ...
    computeHeliocentricPosition(planet, timeDays) {
      const centuries = timeDays / 36525;
      const elements = window.SimCore.computeElements(planet, centuries);
      ...
      const M = window.SimCore.normalizeRadians(meanAnomaly);
      const E = window.SimCore.solveKepler(M, elements.e);
    ...
    computeMinCameraDistance(radius, vFovRad, aspect, fillRatio) {
